...
function buildSystemPrompt(mode: ChatMode, scriptContent: string, objections: any[], language: ChatLanguage): string {
  ...
  const langInstruction = LANG_LABELS[language]?.instruction || LANG_LABELS.ru.instruction;
...
serve(async (req) => {
  ...
  try {
    ...
    const validModes: ChatMode[] = ["ai_manager", "ai_client", "ai_auto"];
    const validLangs: ChatLanguage[] = ["ru", "en", "uk"];
    const safeMode = validModes.includes(mode as ChatMode) ? (mode as ChatMode) : "ai_client";
    const safeLang = validLangs.includes(language as ChatLanguage) ? (language as ChatLanguage) : "ru";

    const systemPrompt = buildSystemPrompt(safeMode, scriptContent, objections, safeLang);
